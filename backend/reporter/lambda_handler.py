...
async def run_reporter_agent(
    ...
    with trace("Reporter Agent"):
        agent = Agent[ReporterContext](  # Specify the context type
            name="Report Writer", instructions=sanitize_user_input(REPORTER_INSTRUCTIONS), model=model, tools=tools
        )

        result = await Runner.run(
            agent,
            input=sanitize_user_input(task),
        ...
        if observability:
            evaluation = await evaluate(sanitize_user_input(REPORTER_INSTRUCTIONS), sanitize_user_input(task), response)
            score = evaluation.score / 100
            comment = evaluation.feedback
            observation = f"Score: {score} - Feedback: {comment}"
            logger.info(f"[Judge] {observation}")
            try:
                ...
                logger.warning(f"[Judge] Could not send score to LangFuse: {e}")
