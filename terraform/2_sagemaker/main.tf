terraform {
  ...
  # Enables CI/CD (GitHub Actions) and team collaboration by storing
  # state centrally instead of on a developer's local machine.
  # The state bucket and lock table are created by terraform/0_bootstrap.
  backend "s3" {
    bucket         = "alex-tfstate-064102991378"
    key            = "2_sagemaker/terraform.tfstate"
    region         = "eu-west-1"
