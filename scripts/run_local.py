...
def cleanup(signum=None, frame=None):
    ...
    for proc in processes:
        try:
            # On Windows, proc.terminate() with shell=True only kills cmd.exe,
            # not the child processes. Use taskkill /T to kill the whole tree.
...
def check_requirements():
    ...
    # Check npm (use shell=True on Windows because npm is a .cmd wrapper)
    try:
...
def start_frontend():
    ...
    # Start the frontend (shell=True needed on Windows where npm is a .cmd wrapper)
    proc = subprocess.Popen(
        ["npm", "run", "dev"],
        cwd=frontend_dir,
        stdout=subprocess.PIPE,
    ...
    # Read stdout in a background thread to avoid blocking (select doesn't work with pipes on Windows)
    output_lines = []
